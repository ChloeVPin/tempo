...
- Mutation testing on `src/core/civil.ts` via Stryker (`npm run test:mutation`): a **90.2% recorded run** (256–263 kills across later local runs), zero meaningful survivors in the civil math core; `dayOfYear`/`quarter`/ISO-week error contracts now asserted. Dev deps: `@stryker-mutator/core`, `@stryker-mutator/vitest-runner`, `@stryker-mutator/typescript-checker` (all pinned 10.0.0).
